<template>
  <div class="projects-overview" :class="`mobile-${mobileTab}`">
    ...
    <div v-if="loading" class="empty-state" role="status">Loading operations…</div>
    <template v-else>
      ...
      <template v-if="data">
        <section class="summary-ribbon" aria-label="Fleet summary">
          ...
          <div><span>Steps delivered <em>{{ displayRange }}</em></span><strong>{{ data.completedSteps }}</strong></div>
          <div :title="`${data.execution.observedWakes} / ${data.execution.outcomes} wakes measured`"><span>Productive wakes <em>{{ displayRange }}</em></span><strong>{{ data.execution.productiveRate == null ? '—' : data.execution.productiveRate.toFixed(0) + '%' }}</strong><small class="coverage">{{ data.execution.coveragePct }}% coverage</small></div>
          <div><span>Model spend <em>{{ displayRange }}</em></span><strong>{{ money(data.modelSpendUsd) }}</strong></div>
          <div><span>External spend <em>{{ displayRange }}</em></span><strong>{{ money(data.externalSpendUsd) }}</strong></div>
        ...
        <section class="project-section" aria-label="Project operations">
          ...
          <div ref="lanesElement" class="lanes" data-testid="project-lanes">
            ...
            <article v-for="p in visibleRows" :key="p.projectID" class="project-lane" :class="{ 'has-attention': needsAttention(p) }" :data-project-id="p.projectID">
              ...
              <button v-if="!shelved" class="result-cell" :aria-label="`Inspect result for ${p.name}`" @click="openInspector([p.projectID])"><div class="result-copy"><small :title="p.result.name">{{ p.result.name }}<span v-if="p.result.selection === 'pinned'"> · pinned</span></small><div><strong>{{ resultValue(p.result) }}</strong><span v-if="p.result.delta != null" class="delta" :class="deltaClass(p.result)">{{ p.result.delta > 0 ? '+' : '' }}{{ number(p.result.delta) }}</span></div><span class="result-meta" :class="{ warning: p.result.stale || p.result.validity !== 'Valid' }">{{ resultMeta(p.result) }}</span></div><ProjectsResultSparkline :name="p.result.name" :history="p.result.history" :class="deltaClass(p.result)" /></button>
              <div v-else class="result-cell"><button :disabled="busy" @click="unarchive(p.projectID)">Unshelve</button></div>
              <div class="activity-cell"><div class="activity-strip" role="img" :aria-label="activitySummary(p)"><div v-for="s in p.series" :key="s.date" class="activity-bin" :title="`${time(s.date)}: ${s.completed} completed, ${s.failed} failed, ${s.deferred} deferred, ${s.cancelled} cancelled`"><i v-if="s.completed" class="completed" :style="{ flex: s.completed }"></i><i v-if="s.failed" class="failed" :style="{ flex: s.failed }"></i><i v-if="s.deferred" class="deferred" :style="{ flex: s.deferred }"></i><i v-if="s.cancelled" class="cancelled" :style="{ flex: s.cancelled }"></i></div></div><small>{{ p.series.reduce((n, s) => n + s.completed + s.failed + s.deferred + s.cancelled, 0) }} recorded wakes</small></div>
              <div class="cost-cell"><strong>{{ money(p.rangeSpendUsd + p.rangeMoneySpendUsd) }}</strong><div class="budget-track" :title="`Lifetime model spend ${money(p.tokenSpendUsd)} / ${money(p.tokenBudgetUsd)} budget`"><i :style="{ width: budgetPct(p) + '%' }" :class="{ warning: budgetPct(p) >= 80 }"></i></div><small>{{ money(p.tokenSpendUsd) }} / {{ money(p.tokenBudgetUsd) }} model lifetime</small></div>
            </article>
          </div>
...
const { range, data, loading, refreshing, error, now, connected, refresh } = useProjectsOverview();
